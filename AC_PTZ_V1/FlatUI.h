...
private:
    static QScopedPointer<FlatUI> self; // 静态指针，用于保存 FlatUI 的单例实例
...
public:
    ...
    static QString setScrollBarQss(QWidget* scroll,                                 //滚动条对象
        ...
        int min = 120,                                   //指示器最小长度
        int max = 12,                                    //滚动条最大长度
        const QString& bgColor = "#e43f1e",              //背景色
    ...
    // 设置复选框样式
    static QString setCheckBoxQss(QCheckBox* checkbox,
        QFont font, // 字体
        int indicatorRadius = 8,                     // 指示器半径
        int indicatorBorderRadius = 0,  // 指示器的圆角半径
        const QString& normalColor = "#D7DBDE",      // 正常颜色
        const QString& checkColor = "#34495E");      // 选中颜色

    // 设置 QLabel 样式
    static QString setLabelQss(QLabel* label,
        const QFont& font,
        int borderRadius = 3,
        const QString& bgColor = "#428bca",
